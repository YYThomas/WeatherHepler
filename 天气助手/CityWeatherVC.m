...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.dtWeatherView = [detailWeatherView detailWeatherView];
    NSLog(@"%s",__func__);
...
-(void)setupWithCityName{
    ...
    [manager GET:urlStr parameters:parameter progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        self.dtWeatherView.currentTemp.text = dict1[@"data"][@"wendu"];
...
-(void)returnBack{
    searchCityViewController *scVC = [[searchCityViewController alloc] init];
