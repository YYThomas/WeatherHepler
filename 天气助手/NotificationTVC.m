...
+ (instancetype)initTableViewCell:(UITableView *)tableVieW andCellIdentify:(NSString *)cellIdentify{
    reminderTableViewCell *cell = [tableVieW dequeueReusableCellWithIdentifier:cellIdentify];
    if (cell == nil) {
        [cell.dayStr sizeToFit];
        [cell.timeStr sizeToFit];
        [cell.weekStr sizeToFit];
        cell = [[[NSBundle mainBundle] loadNibNamed:@"reminderTableViewCell" owner:nil options:nil] lastObject];
